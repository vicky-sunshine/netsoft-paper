...
\subsubsection{DHCP}
% The DHCP Service automatically provide the IP address to DHCP client.
% To implement the function, the controller handle the DCHP packets ( UDP protocol with source port 68 and destination port67)
% and the workflow is as following:
% (1) Waiting for DHCP client send DHCP discover request.
% (2) DHCP server (SDN controller) generate DHCP offer packet and send IP address to DHCP client.
% (3) Server Receive the DHCP request from client, and then generate the DHCP acknowledgement send to client.
...
\begin{figure}[!t]
\centering
\includegraphics[width=3in]{./figures/multi_table}
...
As shown in Table  , all the actions of flow entries are drop. The 1st rule illustrates that SSH connection with source IP address 192.168.2.1 would be blocked. The 2nd rule shows the flow entry would block the Telnet protocol.
...
\begin{table*}[]
\centering
\caption{Firewall table entry}
\label{table:firewall}
\begin{tabular}{|l|l|l|l|l|l|}
\hline
IP proto         & IP Src     & IP Dst & L4 sport & L4 dport & Action                                                                              \\ \hline
TCP        192.168.2.1        & * & *         & 22     & 2000     & \begin{tabular}[c]{@{}l@{}}IP Dst $\,\to\,$ 192.168.8.40; \\ L4 dport $\,\to\,$ 5566\end{tabular} \\ \hline
140.114.71.178 & 10.10.10.1 & TCP         & 8976     & 2001     & \begin{tabular}[c]{@{}l@{}}IP Src $\,\to\,$ 192.168.8.80; \\ L4 sport $\,\to\,$ 6979\end{tabular}   \\ \hline
\end{tabular}
\end{table*}
